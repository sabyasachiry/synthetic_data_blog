...
Despite the growth of AI, there are considerable barriers to overcome. Quality of data used to train a model is seen as the biggest hurdle in AI implementation in addition to the sub issues including data collection and data imbalance. Data confidentiality and data regulation is also causing bottleneck situation limiting utility of data. Often organisations have to put in extra resources on time consuming activities such as data collection and data cleansing. Nearly [83% of respondents](https://www.jbs.cam.ac.uk/faculty-research/centres/alternative-finance/publications/transforming-paradigms/) perceive the regulation around sharing personal information or data between jurisdiction to be significnatly problematic for their investment in AI.

The above has hilighted urgency in need to optimize data utility and share data using innovative privacy enhancing technologies to prevent both quality and data sharing concerns.
...
The accuracy and performance matrices generated by Machine Learning models depend on the statistical relationship between the input data. Methods used for data aggregation coupled with sampling size, imbalanced data distribution, and random noise can distort such relationship between the variables impacting on model performance, which may erode trust in processes.

A machine learning model is reproducible if one can fully replicate the exact results reported. Reproducibility of model output is critical in order to develop algorithms to reliably solve complex tasks at scale, with limited or no human supervision [2]. Adding synthetic data can help improve both reproducibility and predictive perforamnce of the model making it more inclusive and fair. Synthetic data can help overcome issues with sampling size, imbalanced in subpopulation and noise reduction within dataset ensuring complete seperation of training and evaluation dataset with no data leakag.[3]

For example many machine learning algorithms are used in design and implementation of Intrusion Detection Systems(IDS) for computer networks to identify and response to several types of intrusion/attacks. Using multiclass classification and clustering algorithm the model aims to predict users' behaviour on system.

However, datasets used in developing such systems are often imbalanced due to not enough real world intrusion/attack data available for training. Which increases the false positive and/or false negative outcome. Furthermore, the sophesticated tools used in launching attacks make it difficult to predict such attacks based on actual data.

A research experiement[4] shows that synthetic data smples generated using sophisticated technique Conditional Generative Adversarial Network (CTGAN) can balance the training samples, using both actual and unforeseen simulations, can increase prediction accuracy by as much as 8%.
