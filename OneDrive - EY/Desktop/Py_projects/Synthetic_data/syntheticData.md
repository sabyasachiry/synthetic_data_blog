...
For example, AI is widely used in design and implementation of Intrusion Detection Systems(IDS) in order to quickly identify and produce timely response to attacks on the computer network. The algorithm is typically trained to predict users' behaviour, using multiclass classification and clustering techniques, to report unusual pattern from the normal one. Privacy and reputational concerns are additional factors which rarely allow infosec personel within organizations share attack's data with others. Therefore lack of data makes it nearlly impossible to train IDS on a balanced dataset covering all possible scenarios. The algorithm trained on such imbalanced dataset generates higher false positives and/or false negatives.

A research experiement shows that synthetic data samples generated using sophisticated technique Conditional Generative Adversarial Network (CTGAN) can balance the training samples, using both actuals and simulations, can increase prediction accuracy by as much as 8% [4].
