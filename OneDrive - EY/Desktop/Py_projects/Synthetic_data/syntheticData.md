...
Reproducibility of the model outcome is also an important measure. An outcome of a machine learning model is reproducible if one can fully replicate the exact results which is critical in order to develop algorithms to reliably solve complex tasks at scale, with limited or no human supervision [2]. Synthetic data often helps overcome issues of sampling size and data imbalance in subpopulation ensuring complete sepeartion of training and evaluation datasets. Which helps improve both reproducibility and predicitve performance of a model making it more inclusive and fair[3].

For example, AI is widely used in design and implementation of Intrusion Detection Systems(IDS) in order to quickly identify and produce timely response to attacks on the computer networks. The algorithm is typically trained to predict users' behaviour on the network, using multiclass classification and clustering techniques, to identify unusual pattern from the normal one. Information security personel within organizations rarely share such data in public due to the privacy and reputational concerns. It is nearlly impossible to train IDS on a dataset balancing all possible scenarios. Imbalance in dataset used for training IDS systems generate more false positives and false negatives.

lack of actual data on attacks/intrusion makes the training dataset used to train algorithms quite imbalanced in nature.

 Which increases the false positive and/or false negative outcome. Furthermore, the sophesticated tools used in launching attacks make it difficult to predict such attacks based on actual data.

A research experiement shows that synthetic data smples generated using sophisticated technique Conditional Generative Adversarial Network (CTGAN) can balance the training samples, using both actual and unforeseen simulations, can increase prediction accuracy by as much as 8% [4].
